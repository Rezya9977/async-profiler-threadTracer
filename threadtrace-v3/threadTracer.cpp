...
/**
 * 通过最长公共前缀识别算法对线程名分组的功能大致已经完成
 * 仍有一些TODO List：
 * TODO:1. 代码结构改造、优化
 *      2. 新增通过配置命名规则的方式，来根据命名规则识别线程，与算法相结合
*/
...
void groupbyThread(
            std::unordered_map<std::string, jint> threadStates,
            std::unordered_map<std::string, jvmtiThreadInfo> threadInfos, 
    ...
    traverseTrieDFS(trie.getRoot(), "", currentBranchSerialNumber, trie.getRoot(), commonLongestPrefix, completedBranches);
    ...
    for (auto it = result.begin(); it != result.end(); ++it) {
        ...
        for (auto itItem = group.begin(); itItem != group.end(); ++itItem) {
            ...
            std::cout << "    " << item << "    " << "state:" << getJavaThreadState(threadStates[item]) << std::endl;
        }
        std::cout << std::endl;
    }
...
void statisticThreads(jvmtiEnv* jvmti) {
    ...
    for (int i = 0; i < threads_count; i++) {
        ...
        threadInfos[info.name] = info;
        thread_names.push_back(info.name);
        threadStates[info.name] = thread_state_ptr;
        // std::cout << "插入*threads->name:" << info.name << std::endl;
    }
    //把threads_trie和thread_names传递给groupbythread函数
    groupbyThread(threadStates, threadInfos, threads_trie, thread_names, commonLongestPrefix, completedBranches);
    // 重置threads_trie和thread_names
    // *threads_trie = Trie();
    commonLongestPrefix.clear();
