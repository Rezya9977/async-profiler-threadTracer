...
void traverseTrieDFS(
    ...
    for (auto& entry : node->children) {
        ...
        if (completedBranches.find(currentBranchSerialNumber) != completedBranches.end()) {
            traverseTrieDFS(entry.second, commonPrefix + ch, currentBranchSerialNumber + 1, rootNode, commonLongestPrefix, completedBranches);
        }
        else {
            traverseTrieDFS(entry.second, commonPrefix + ch, currentBranchSerialNumber, rootNode, commonLongestPrefix, completedBranches);
        }
    }
